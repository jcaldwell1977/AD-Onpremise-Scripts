...
<head>
    ...
    <style>
        ...
        body { font-family: 'Segoe UI', Arial, sans-serif; margin: 0; padding: 40px; background: #f8f9fa; color: #333; line-height: 1.6; }
        .container { max-width: 1300px; margin: 0 auto; background: white; padding: 40px; border-radius: 12px; box-shadow: 0 4px 25px rgba(0,0,0,0.12); }
        h1 { color: #003366; text-align: center; border-bottom: 4px solid #003366; padding-bottom: 20px; margin-bottom: 30px; }
        h2 { color: #003366; border-left: 6px solid #003366; padding-left: 18px; background: #f0f4f8; padding: 12px 0 12px 18px; margin-top: 45px; }
        h3 { color: #0055aa; margin-top: 35px; }
        table { width: 100%; border-collapse: collapse; margin: 18px 0; }
        th, td { padding: 12px 15px; border: 1px solid #ccc; text-align: left; }
        th { background-color: #003366; color: white; }
        tr:nth-child(even) { background-color: #f9f9f9; }
        .section { margin-bottom: 55px; }
        .header-info { text-align: center; margin-bottom: 40px; color: #555; font-size: 1.1em; }
        .footer { text-align: center; margin-top: 80px; color: #777; font-size: 0.95em; }
...
$Html += @"
        <div class="section">
            ...
            <table><tr><th>Server Name</th><th>Roles</th></tr>
"@
try {
...
try {
    ...
    if ($GatewayConfig) {
        $Html += "<h3>Gateway Settings</h3><table><tr><th>Property</th><th>Value</th></tr>"
...
if (-not $SessionHosts) { $SessionHosts = @($env:COMPUTERNAME) }
...
# Export CSVs
Get-RDServer | Export-Csv "$ReportFolder\RDS_Servers.csv" -NoTypeInformation -ErrorAction SilentlyContinue
